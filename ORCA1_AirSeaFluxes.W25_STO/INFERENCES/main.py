...
def ocean_info():
    ...
    tunnel_config.append( { 'label' : 'TO_NEMO_FIELDS', \
                            'grids' : { 'ORCA1' : {'npts' : (360,331)} }, \
                            'exchs' : [ {'freq' : 14400, 'grd' : 'ORCA1', 'lvl' : 1, \
                                         'in' : ['ux','uy','toce','tair','p','q','sto_tau','sto_heat'], 'out' : ['taux','tauy','Qs','Ql']} ] }
                        ) 
...
def production():
    ...
    def loop_core(**inputs):
        packed_res = W25ann( inputs['ux'], inputs['uy'], inputs['toce'], inputs['tair'], inputs['p'], inputs['q'], inputs['sto_tau'], inputs['sto_heat'] )
