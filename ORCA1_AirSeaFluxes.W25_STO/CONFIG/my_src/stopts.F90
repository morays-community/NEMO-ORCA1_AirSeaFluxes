MODULE stopts
   ...
   REAL(wp), PUBLIC, DIMENSION(:,:,:),   ALLOCATABLE :: sto2d_loc      ! 2D stochastic parameters
   REAL(wp), PUBLIC, DIMENSION(:,:,:,:,:), ALLOCATABLE :: pts_ran_loc
           ...
           DO jk = 1, jpkm1
              ...
              DO jj = 1, jpj
                 ...
                 DO ji = 1, jpi
                    ...
                    zdts   = ( zdtsip + zdtsim ) * sto2d_loc(ji,jj,jsto_eosi(jdof)) + &
                           & ( zdtsjp + zdtsjm ) * sto2d_loc(ji,jj,jsto_eosj(jdof)) + &
                           & ( zdtskp + zdtskm ) * sto2d_loc(ji,jj,jsto_eosk(jdof))
!                   zdts   = zdtsip * MAX(sto2d_loc(ji,jj,jsto_eosi),0._wp) + &
!                          & zdtsim * MIN(sto2d_loc(ji,jj,jsto_eosi),0._wp) + &
!                          & zdtsjp * MAX(sto2d_loc(ji,jj,jsto_eosj),0._wp) + &
!                          & zdtsjm * MIN(sto2d_loc(ji,jj,jsto_eosj),0._wp) + &
!                          & zdtskp * MAX(sto2d_loc(ji,jj,jsto_eosk),0._wp) + &
!                          & zdtskm * MIN(sto2d_loc(ji,jj,jsto_eosk),0._wp)
                    zdts   = zdts  * tmask(ji,jj,jk) *SIN( gphit(ji,jj) * rad )
                    pts_ran_loc(ji,jj,jk,jts,jdof) = zdts * 0.5_wp
                    !
                  END DO
               END DO
            END DO
      ...
      DO jdof = 1, nn_sto_eos
         DO_3D( 1, 1, 1, 1, 1, jpkm1 )
            pts_ran_loc(ji,jj,jk,jp_sal,jdof) = MIN( ABS(pts_ran_loc(ji,jj,jk,jp_sal,jdof)) ,  &
                                          &      MAX(pts(ji,jj,jk,jp_sal),0._wp) )     &
                                          &  * SIGN(1._wp,pts_ran_loc(ji,jj,jk,jp_sal,jdof))
         END_3D
      END DO
...
!           DO jj = 1, jpj
!              DO ji = 1, jpi
!                 pts_ran_loc(ji,jj,jk,jp_tem,jdof) = MIN( ABS(pts_ran_loc(ji,jj,jk,jp_tem,jdof)) ,    &
!                                               &      MAX(pts(ji,jj,jk,jp_tem)+2._wp,0._wp) ) &
!                                               &  * SIGN(1._wp,pts_ran_loc(ji,jj,jk,jp_tem,jdof))
!              END DO
!           END DO
!        END DO
!     END DO
      ...
      ! Lateral boundary conditions on pts_ran_loc
      DO jdof = 1, nn_sto_eos
         DO jts = 1, jpts
            CALL lbc_lnk( 'stopts', pts_ran_loc(:,:,:,jts,jdof), 'T' , 1._wp )
         END DO
      END DO
   ...
   SUBROUTINE sto_pts_init
      ...
      ALLOCATE(pts_ran_loc(jpi,jpj,jpk,jpts,nn_sto_eos))
